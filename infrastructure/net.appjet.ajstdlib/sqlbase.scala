...
class SQLBase(driverClass: String, url: String, userName: String, password: String) {
  ...
  def isPg:Boolean    = (url.startsWith("jdbc:postgresql:"));
...
//   }
  ...
  def autoIncrementClause = if (isDerby) "GENERATED BY DEFAULT AS IDENTITY" 
                            else if (isMysql) "AUTO_INCREMENT"
                            else if (isPg) "SERIAL";
